...
class Api::V1::StocksController < ApplicationController
  ...
  def create
    if !(authenticate_if_admin!) && authenticate_trader_status! 
      ...
      transaction = trader.transaction_histories.new(shares: params[:stock][:shares],price_per_unit: market.price_per_unit,total_price: stock.total_price, trader: trader, stock_name: market.stock_name, balance: trader.wallet, transaction_type: "buy", symbol: market.symbol)
  ...
  def buy_update
    if !(authenticate_if_admin!) && authenticate_trader_status!
      ...
      if single_stock.update(shares: new_stock_shares, total_price: new_total_price)
        transaction = trader.transaction_histories.new(shares: params[:shares], price_per_unit: market.price_per_unit, total_price: single_stock.total_price, trader: trader, stock_name: market.stock_name, balance: trader.wallet, transaction_type: "buy", symbol: market.symbol)
        if transaction.save
  ...
  def sell_update
    if !(authenticate_if_admin!) && authenticate_trader_status!
      ...
      if new_stock_shares <= 0
        transaction = trader.transaction_histories.create(shares: params[:shares],price_per_unit: market.price_per_unit,total_price: single_stock.total_price, trader: trader, stock_name: market.stock_name, symbol: market.symbol, balance: trader.wallet, transaction_type: "sell")
      ...
      if single_stock.update(shares: new_stock_shares, total_price: new_stock_shares * single_stock.price_per_unit)
       transaction = trader.transaction_histories.new(shares: params[:shares], price_per_unit: market.price_per_unit, total_price: single_stock.total_price, trader: trader, stock_name: market.stock_name, symbol:market.symbol, balance: trader.wallet, transaction_type: "sell")
        if transaction.save 
          render json: trader.transaction_histories.all, status: 200
        else
  ...
  def check_existing_stock
    if current_api_v1_user.trader.stocks.all.length > 0 
      current_api_v1_user.trader.stocks.all.each do |stock|
        if stock.market_id == params[:stock][:market_id]
          return true
        end
      end
    else
