...
RSpec.describe 'Admin API Testing', type: :request do
  ...
  let(:invalid_admin_attributes) do
    { admin: {
      name: nil,
      email: nil,
      password: '24242424',
  ...
  let(:valid_trader_attributes) do
    {
      trader: {
        name: 'Leann',
        email: 'ellerreyes82@gmail.com',
  ...
  let(:invalid_trader_attributes) do
    {
      trader: {
        name: nil,
        email: nil,
  ...
  it '5. It should be able to update an admin (admins#update)' do
    patch "/api/v1/admins/#{@admin.id}", params: { admin: { name: 'Stanley', email: 'stanleytarce18@gmail.com' } },
                                         headers: @headers
    expect(response).to have_http_status(:success)
